...
if __name__ == "__main__":
    ...
    auction_thread = threading.Thread(target=auction_manager.run)
    ...
    auction_demo = Auction(Order("A0", "sell", "Gadget", 5, 60.0), duration=5)
    auction_manager.active_auctions["Gadget"] = auction_demo
    auction_demo.add_bid(Order("A1", "buy", "Gadget", 5, 75.0))
    auction_demo.add_bid(Order("A2", "buy", "Gadget", 5, 80.0))
    ...
    for _ in range(pool_size):
        t = threading.Thread(target=bot_worker, args=(engine, product_names))
        threads.append(t)
        t.start()
